module.exports = () => {
  $.gulp.task('prepareHtmlBuild', function() {
    ...
    for (let i = 0; i < templates.length; i++) {
      ...
      const file = $.fs
        .readFileSync($.config.sourcePath + '/' + $.config.hbsPath + '/' + templateName + '.hbs')
        .toString()
    ...
    for (let k = 0; k < tmpFiles.length; k++) {
      ...
      if ($.argv._[0] === 'build') {
        $.fs.writeFileSync(
          ...
          hbs.replace(/<title>(.*)/, '<title>' + pageNames[tpmTemplateName] + '</title>'),
        )
      } else {
    ...
    const templateFile = $.fs.readFileSync('./config/template-build.html').toString()
    $.fs.writeFileSync(
      $.config.tmpPath + '/' + 'html/index.html',
      templateFile.replace('{{items}}', html).replace(/{{siteName}}/g, $.config.siteName),
    )
